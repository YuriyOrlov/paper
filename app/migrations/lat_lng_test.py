...
def yandex_geocode(request):
    ...
    response_result = query_result.get('response')\
                                  .get('GeoObjectCollection')\
                                  .get('metaDataProperty')\
                                  .get('GeocoderResponseMetaData')\
                                  .get('found')
    if int(response_result) > 0:
        lat_lng_string = query_result.get('response')\
                                     .get('GeoObjectCollection')\
                                     .get('featureMember')[0]\
                                     .get('GeoObject')\
                                     .get('Point')\
...
def adding_coords(counter):
    ...
    query = Item.query.filter(func.length(Item.obj_address) >= 20).filter(Item.lat == None).filter(Item.lng == None)
    objects_without_coords_id = [x.id for x in query.distinct()]
    db_session.close()
