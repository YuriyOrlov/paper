...
>>>>>>> 27c45050cc1c2e9def9983164f8900d742a187c1
    ...
    elif (object_characteristics_tags is None) and (parsed_data.status_code != 404):
        ...
        information_about_floors_received_from_description = retrieving_floors[0] if len(
            retrieving_floors) != 0 else None

        if information_about_floors_received_from_description and (
                '/' in information_about_floors_received_from_description):
        ...
        if type_of_object == 'RS':
            if ('квартира' in name_node.text.strip()) and ('продажи' in name_node.text.strip()):
                total_space_received_from_description = re.findall(
                    'квартира\s+([0-9/\\\\.,]+)', name_node.text.strip())[0]
                living_space_received_from_description = re.findall(
                    'продажи\s+([0-9/\\\\.,]+)', name_node.text.strip())[0]
